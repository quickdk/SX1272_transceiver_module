...
/*
 Function: Sets the signal output pin in the module.
 Returns: Integer that determines if there has been any error
   state = 2  --> The command has not been executed
   state = 1  --> There has been an error while executing the command
   state = 0  --> The command has been executed with no errors
   state = -1 --> Forbidden command for this protocol
 Parameters:
   p: Output pin type to set in configuration.
*/
int8_t SX1272::setRfOutPin(char p)
{
	byte st0;
  int8_t state = 2;
  byte value = 0x00;
  ...
  #if (SX1272_debug_mode > 1)
	  Serial.println();
	  Serial.println(F("Starting 'setRfrOutPin'"));
  #endif
  ...
  switch (p)
  {
    ...
    case 0:  _rfOutPin = 0x00; //RFO
               break;
  
    case 1:  _rfOutPin = 0x80; //PA_BOOST
               break;
  ...
  writeRegister(REG_PA_CONFIG, _power | _rfOutPin);	// Setting output pin
  value = readRegister(REG_PA_CONFIG);

  if( value == _power | _rfOutPin)
  {
	  state = 0;
	  #if (SX1272_debug_mode > 1)
		  Serial.println(F("## Output pin has been successfully set ##"));
		  Serial.println();
	  #endif
  }
  else
...
int8_t SX1272::setPower(char p)
{
  ...
  switch (p)
  {
    ...
    // H = high
    // M = max
	// P = max using PA_BOOST (+20dBm) - only works on modules with PA_BOOST connected! (will fry the output if not)

    case 'P':  _power = 0x0F;
			   _PaDac = 0x07; //enables +20dBm output 
               break;
  ...
  writeRegister(REG_PA_CONFIG, _power | _rfOutPin);	// Setting output power value
  value = readRegister(REG_PA_CONFIG);
  
  value2 = readRegister(REG_PA_DAC) & 0xF8; // Retain default value on reserved bits (3-7). Set bits 2-0 to 0
  writeRegister(REG_PA_DAC, value2 | _PaDac); // Enable or disable 20dBm output
  value2 = readRegister(REG_PA_DAC);

  if((value == _power | _rfOutPin) && (value2 == value2 | _PaDac))
  {
	  state = 0;
	  #if (SX1272_debug_mode > 1)
